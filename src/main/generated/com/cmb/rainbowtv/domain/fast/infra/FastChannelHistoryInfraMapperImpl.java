...
import com.cmb.rainbowtv.domain.fast.infra.data.entity.FastChannelHistoryEntity;
import com.cmb.rainbowtv.domain.fast.model.FastChannel;
import com.cmb.rainbowtv.domain.fast.model.FastChannelHistory;
...
@Component
public class FastChannelHistoryInfraMapperImpl implements FastChannelHistoryInfraMapper {

    @Override
    public FastChannelHistory toFastChannelHistory(FastChannelHistoryEntity entity) {
        if ( entity == null ) {
            return null;
        }

        FastChannelHistory.FastChannelHistoryBuilder<?, ?> fastChannelHistory = FastChannelHistory.builder();

        fastChannelHistory.createdAt( entity.getCreatedAt() );
        fastChannelHistory.updatedAt( entity.getUpdatedAt() );
        fastChannelHistory.fchIdx( entity.getFchIdx() );
        fastChannelHistory.fastChannel( fastChannelEntityToFastChannel( entity.getFastChannel() ) );
        fastChannelHistory.member( memberEntityToMember( entity.getMember() ) );

        return fastChannelHistory.build();
    }

    @Override
    public FastChannelHistoryEntity toFastChannelHistoryEntity(FastChannelHistory model) {
        if ( model == null ) {
            return null;
        }

        FastChannelHistoryEntity.FastChannelHistoryEntityBuilder fastChannelHistoryEntity = FastChannelHistoryEntity.builder();

        fastChannelHistoryEntity.fchIdx( model.getFchIdx() );
        fastChannelHistoryEntity.fastChannel( fastChannelToFastChannelEntity( model.getFastChannel() ) );
        fastChannelHistoryEntity.member( memberToMemberEntity( model.getMember() ) );

        return fastChannelHistoryEntity.build();
    }
