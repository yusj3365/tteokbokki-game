...
import com.cmb.rainbowtv.domain.authority.infra.data.entity.AdminCmsMenuEntity;
import com.cmb.rainbowtv.domain.authority.model.AdminCmsMenu;
...
@Component
public class AdminCmsMenuInfraMapperImpl implements AdminCmsMenuInfraMapper {

    @Override
    public List<AdminCmsMenuEntity> toAdminCmsMenuEntityList(List<AdminCmsMenu> adminCmsMenus) {
        if ( adminCmsMenus == null ) {
            return null;
        }

        List<AdminCmsMenuEntity> list = new ArrayList<AdminCmsMenuEntity>( adminCmsMenus.size() );
        for ( AdminCmsMenu adminCmsMenu : adminCmsMenus ) {
            list.add( adminCmsMenuToAdminCmsMenuEntity( adminCmsMenu ) );
        }
    ...
    @Override
    public List<AdminCmsMenu> toAdminCmsMenuList(List<AdminCmsMenuEntity> adminCmsMenuEntities) {
        if ( adminCmsMenuEntities == null ) {
            return null;
        }

        List<AdminCmsMenu> list = new ArrayList<AdminCmsMenu>( adminCmsMenuEntities.size() );
        for ( AdminCmsMenuEntity adminCmsMenuEntity : adminCmsMenuEntities ) {
            list.add( toAdminCmsMenu( adminCmsMenuEntity ) );
        }
    ...
    @Override
    public AdminCmsMenu toAdminCmsMenu(AdminCmsMenuEntity adminCmsMenuEntity) {
        if ( adminCmsMenuEntity == null ) {
            return null;
        }

        AdminCmsMenu.AdminCmsMenuBuilder<?, ?> adminCmsMenu = AdminCmsMenu.builder();

        adminCmsMenu.createdAt( adminCmsMenuEntity.getCreatedAt() );
        adminCmsMenu.updatedAt( adminCmsMenuEntity.getUpdatedAt() );
        adminCmsMenu.acmIdx( adminCmsMenuEntity.getAcmIdx() );
        adminCmsMenu.member( memberEntityToMember( adminCmsMenuEntity.getMember() ) );
        adminCmsMenu.cmsAuthorityDefine( cmsAuthorityDefineEntityToCmsAuthorityDefine( adminCmsMenuEntity.getCmsAuthorityDefine() ) );

        return adminCmsMenu.build();
    }
    ...
    protected CmsAuthorityDefineEntity cmsAuthorityDefineToCmsAuthorityDefineEntity(CmsAuthorityDefine cmsAuthorityDefine) {
        if ( cmsAuthorityDefine == null ) {
            return null;
        }

        CmsAuthorityDefineEntity.CmsAuthorityDefineEntityBuilder cmsAuthorityDefineEntity = CmsAuthorityDefineEntity.builder();

        cmsAuthorityDefineEntity.cadIdx( cmsAuthorityDefine.getCadIdx() );
        cmsAuthorityDefineEntity.cadDepth( cmsAuthorityDefine.getCadDepth() );
        cmsAuthorityDefineEntity.cadParent( cmsAuthorityDefine.getCadParent() );
        cmsAuthorityDefineEntity.cadName( cmsAuthorityDefine.getCadName() );
        cmsAuthorityDefineEntity.cadRoute( cmsAuthorityDefine.getCadRoute() );

        return cmsAuthorityDefineEntity.build();
    }

    protected AdminCmsMenuEntity adminCmsMenuToAdminCmsMenuEntity(AdminCmsMenu adminCmsMenu) {
        if ( adminCmsMenu == null ) {
            return null;
        }

        AdminCmsMenuEntity.AdminCmsMenuEntityBuilder adminCmsMenuEntity = AdminCmsMenuEntity.builder();

        adminCmsMenuEntity.acmIdx( adminCmsMenu.getAcmIdx() );
        adminCmsMenuEntity.member( memberToMemberEntity( adminCmsMenu.getMember() ) );
        adminCmsMenuEntity.cmsAuthorityDefine( cmsAuthorityDefineToCmsAuthorityDefineEntity( adminCmsMenu.getCmsAuthorityDefine() ) );

        return adminCmsMenuEntity.build();
    }
    ...
    protected CmsAuthorityDefine cmsAuthorityDefineEntityToCmsAuthorityDefine(CmsAuthorityDefineEntity cmsAuthorityDefineEntity) {
        if ( cmsAuthorityDefineEntity == null ) {
            return null;
        }

        CmsAuthorityDefine.CmsAuthorityDefineBuilder<?, ?> cmsAuthorityDefine = CmsAuthorityDefine.builder();

        cmsAuthorityDefine.createdAt( cmsAuthorityDefineEntity.getCreatedAt() );
        cmsAuthorityDefine.updatedAt( cmsAuthorityDefineEntity.getUpdatedAt() );
        cmsAuthorityDefine.cadIdx( cmsAuthorityDefineEntity.getCadIdx() );
        cmsAuthorityDefine.cadDepth( cmsAuthorityDefineEntity.getCadDepth() );
        cmsAuthorityDefine.cadParent( cmsAuthorityDefineEntity.getCadParent() );
        cmsAuthorityDefine.cadName( cmsAuthorityDefineEntity.getCadName() );
        cmsAuthorityDefine.cadRoute( cmsAuthorityDefineEntity.getCadRoute() );
