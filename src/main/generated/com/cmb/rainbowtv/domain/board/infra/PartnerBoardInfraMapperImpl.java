package com.cmb.rainbowtv.domain.board.infra;

import com.cmb.rainbowtv.domain.board.infra.data.entity.PartnerBoardEntity;
import com.cmb.rainbowtv.domain.board.infra.data.entity.PartnerBoardHistoryEntity;
import com.cmb.rainbowtv.domain.board.model.PartnerBoard;
import com.cmb.rainbowtv.domain.board.model.PartnerBoardHistory;
...
@Component
public class PartnerBoardInfraMapperImpl implements PartnerBoardInfraMapper {

    @Override
    public PartnerBoard toPartnerBoard(PartnerBoardEntity partnerBoardEntity) {
        if ( partnerBoardEntity == null ) {
            return null;
        }

        PartnerBoard.PartnerBoardBuilder<?, ?> partnerBoard = PartnerBoard.builder();

        partnerBoard.createdAt( partnerBoardEntity.getCreatedAt() );
        partnerBoard.updatedAt( partnerBoardEntity.getUpdatedAt() );
        partnerBoard.pbIdx( partnerBoardEntity.getPbIdx() );
        partnerBoard.member( memberEntityToMember( partnerBoardEntity.getMember() ) );
        partnerBoard.pbType( partnerBoardEntity.getPbType() );
        partnerBoard.pbParent( partnerBoardEntity.getPbParent() );
        partnerBoard.pbSubject( partnerBoardEntity.getPbSubject() );
        partnerBoard.pbContent( partnerBoardEntity.getPbContent() );

        return partnerBoard.build();
    }

    @Override
    public PartnerBoardEntity toPartnerBoardEntity(PartnerBoard partnerBoard) {
        if ( partnerBoard == null ) {
            return null;
        }

        PartnerBoardEntity.PartnerBoardEntityBuilder partnerBoardEntity = PartnerBoardEntity.builder();

        partnerBoardEntity.pbIdx( partnerBoard.getPbIdx() );
        partnerBoardEntity.member( memberToMemberEntity( partnerBoard.getMember() ) );
        partnerBoardEntity.pbType( partnerBoard.getPbType() );
        partnerBoardEntity.pbParent( partnerBoard.getPbParent() );
        partnerBoardEntity.pbSubject( partnerBoard.getPbSubject() );
        partnerBoardEntity.pbContent( partnerBoard.getPbContent() );

        return partnerBoardEntity.build();
    }

    @Override
    public PartnerBoardHistoryEntity toPartnerBoardHistoryEntity(PartnerBoardHistory partnerBoardHistory) {
        if ( partnerBoardHistory == null ) {
            return null;
        }

        PartnerBoardHistoryEntity.PartnerBoardHistoryEntityBuilder partnerBoardHistoryEntity = PartnerBoardHistoryEntity.builder();

        partnerBoardHistoryEntity.pbhIdx( partnerBoardHistory.getPbhIdx() );
        partnerBoardHistoryEntity.member( memberToMemberEntity( partnerBoardHistory.getMember() ) );
        partnerBoardHistoryEntity.partnerBoard( toPartnerBoardEntity( partnerBoardHistory.getPartnerBoard() ) );

        return partnerBoardHistoryEntity.build();
    }
