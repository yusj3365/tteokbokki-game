...
import com.cmb.rainbowtv.domain.media.infra.data.entity.MediaHashtagEntity;
...
import com.cmb.rainbowtv.domain.media.model.MediaHashtag;
...
@Component
public class MediaHashTagInfraMapperImpl implements MediaHashTagInfraMapper {

    @Override
    public MediaHashtag toMediaHashtag(MediaHashtagEntity entity) {
        if ( entity == null ) {
            return null;
        }

        MediaHashtag.MediaHashtagBuilder<?, ?> mediaHashtag = MediaHashtag.builder();

        mediaHashtag.createdAt( entity.getCreatedAt() );
        mediaHashtag.updatedAt( entity.getUpdatedAt() );
        mediaHashtag.mhtIdx( entity.getMhtIdx() );
        mediaHashtag.media( mediaEntityToMedia( entity.getMedia() ) );
        mediaHashtag.hashtag( hashtagEntityToHashtag( entity.getHashtag() ) );

        return mediaHashtag.build();
    }

    @Override
    public MediaHashtagEntity toMediaHashtagEntity(MediaHashtag model) {
        if ( model == null ) {
            return null;
        }

        MediaHashtagEntity.MediaHashtagEntityBuilder mediaHashtagEntity = MediaHashtagEntity.builder();

        mediaHashtagEntity.mhtIdx( model.getMhtIdx() );
        mediaHashtagEntity.media( mediaToMediaEntity( model.getMedia() ) );
        mediaHashtagEntity.hashtag( hashtagToHashtagEntity( model.getHashtag() ) );

        return mediaHashtagEntity.build();
    }

    @Override
    public List<MediaHashtag> toMediaHashtagList(List<MediaHashtagEntity> entities) {
        if ( entities == null ) {
            return null;
        }

        List<MediaHashtag> list = new ArrayList<MediaHashtag>( entities.size() );
        for ( MediaHashtagEntity mediaHashtagEntity : entities ) {
            list.add( toMediaHashtag( mediaHashtagEntity ) );
        }
