...
import com.cmb.rainbowtv.domain.log.infra.data.entity.CmsLogEntity;
import com.cmb.rainbowtv.domain.log.model.CmsLog;
...
@Component
public class CmsLogInfraMapperImpl implements CmsLogInfraMapper {

    @Override
    public CmsLogEntity toCmsLogEntity(CmsLog from) {
        if ( from == null ) {
            return null;
        }

        CmsLogEntity.CmsLogEntityBuilder cmsLogEntity = CmsLogEntity.builder();

        cmsLogEntity.clIdx( from.getClIdx() );
        cmsLogEntity.member( memberToMemberEntity( from.getMember() ) );
        cmsLogEntity.clDate( from.getClDate() );
        cmsLogEntity.clIp( from.getClIp() );
        cmsLogEntity.clId( from.getClId() );
        cmsLogEntity.clName( from.getClName() );
        cmsLogEntity.clType( from.getClType() );
        cmsLogEntity.clSo( from.getClSo() );
        cmsLogEntity.clDepth( from.getClDepth() );
        cmsLogEntity.clAccess( from.getClAccess() );
        cmsLogEntity.clAction( from.getClAction() );
        cmsLogEntity.clClient( from.getClClient() );

        return cmsLogEntity.build();
    }

    @Override
    public CmsLog toCmsLog(CmsLogEntity save) {
        if ( save == null ) {
            return null;
        }

        CmsLog.CmsLogBuilder<?, ?> cmsLog = CmsLog.builder();

        cmsLog.createdAt( save.getCreatedAt() );
        cmsLog.updatedAt( save.getUpdatedAt() );
        cmsLog.clIdx( save.getClIdx() );
        cmsLog.member( memberEntityToMember( save.getMember() ) );
        cmsLog.clDate( save.getClDate() );
        cmsLog.clIp( save.getClIp() );
        cmsLog.clId( save.getClId() );
        cmsLog.clName( save.getClName() );
        cmsLog.clType( save.getClType() );
        cmsLog.clSo( save.getClSo() );
        cmsLog.clDepth( save.getClDepth() );
        cmsLog.clAccess( save.getClAccess() );
        cmsLog.clAction( save.getClAction() );
        cmsLog.clClient( save.getClClient() );
