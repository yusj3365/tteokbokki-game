package com.cmb.rainbowtv.domain.content.infra;

import com.cmb.rainbowtv.domain.content.infra.data.entity.ContentRecommendEntity;
import com.cmb.rainbowtv.domain.content.infra.data.entity.ContentRecommendMenuEntity;
import com.cmb.rainbowtv.domain.content.infra.data.entity.ContentSettingEntity;
import com.cmb.rainbowtv.domain.content.model.ContentRecommend;
import com.cmb.rainbowtv.domain.content.model.ContentRecommendMenu;
import com.cmb.rainbowtv.domain.content.model.ContentSetting;
...
@Component
public class ContentInfraMapperImpl implements ContentInfraMapper {

    @Override
    public ContentSetting toContentSetting(ContentSettingEntity entity) {
        if ( entity == null ) {
            return null;
        }

        ContentSetting.ContentSettingBuilder<?, ?> contentSetting = ContentSetting.builder();

        contentSetting.createdAt( entity.getCreatedAt() );
        contentSetting.updatedAt( entity.getUpdatedAt() );
        contentSetting.csIdx( entity.getCsIdx() );
        contentSetting.csType( entity.getCsType() );
        contentSetting.csDisplay( entity.getCsDisplay() );

        return contentSetting.build();
    }

    @Override
    public ContentSettingEntity toContentSettingEntity(ContentSetting contentSetting) {
        if ( contentSetting == null ) {
            return null;
        }

        ContentSettingEntity.ContentSettingEntityBuilder contentSettingEntity = ContentSettingEntity.builder();

        contentSettingEntity.csIdx( contentSetting.getCsIdx() );
        contentSettingEntity.csType( contentSetting.getCsType() );
        contentSettingEntity.csDisplay( contentSetting.getCsDisplay() );

        return contentSettingEntity.build();
    }

    @Override
    public ContentRecommendMenuEntity toContentRecommendMenuEntity(ContentRecommendMenu contentRecommendMenu) {
        if ( contentRecommendMenu == null ) {
            return null;
        }

        ContentRecommendMenuEntity.ContentRecommendMenuEntityBuilder contentRecommendMenuEntity = ContentRecommendMenuEntity.builder();

        contentRecommendMenuEntity.crmIdx( contentRecommendMenu.getCrmIdx() );
        contentRecommendMenuEntity.crmName( contentRecommendMenu.getCrmName() );
        contentRecommendMenuEntity.member( memberToMemberEntity( contentRecommendMenu.getMember() ) );
        contentRecommendMenuEntity.crmSeq( contentRecommendMenu.getCrmSeq() );
        contentRecommendMenuEntity.crmDisplay( contentRecommendMenu.getCrmDisplay() );
        contentRecommendMenuEntity.crmUpdatedat( contentRecommendMenu.getCrmUpdatedat() );
        contentRecommendMenuEntity.displayLeMoGlass( contentRecommendMenu.getDisplayLeMoGlass() );
        contentRecommendMenuEntity.displayLeTb( contentRecommendMenu.getDisplayLeTb() );
        contentRecommendMenuEntity.displayCtvLg( contentRecommendMenu.getDisplayCtvLg() );
        contentRecommendMenuEntity.displayCtvSamsung( contentRecommendMenu.getDisplayCtvSamsung() );
        contentRecommendMenuEntity.crmType( contentRecommendMenu.getCrmType() );

        return contentRecommendMenuEntity.build();
    }

    @Override
    public ContentRecommendMenu toContentRecommendMenu(ContentRecommendMenuEntity save) {
        if ( save == null ) {
            return null;
        }

        ContentRecommendMenu.ContentRecommendMenuBuilder<?, ?> contentRecommendMenu = ContentRecommendMenu.builder();

        contentRecommendMenu.createdAt( save.getCreatedAt() );
        contentRecommendMenu.updatedAt( save.getUpdatedAt() );
        contentRecommendMenu.crmIdx( save.getCrmIdx() );
        contentRecommendMenu.crmName( save.getCrmName() );
        contentRecommendMenu.member( memberEntityToMember( save.getMember() ) );
        contentRecommendMenu.crmSeq( save.getCrmSeq() );
        contentRecommendMenu.crmDisplay( save.getCrmDisplay() );
        contentRecommendMenu.crmUpdatedat( save.getCrmUpdatedat() );
        contentRecommendMenu.displayLeMoGlass( save.getDisplayLeMoGlass() );
        contentRecommendMenu.displayLeTb( save.getDisplayLeTb() );
        contentRecommendMenu.displayCtvLg( save.getDisplayCtvLg() );
        contentRecommendMenu.displayCtvSamsung( save.getDisplayCtvSamsung() );
        contentRecommendMenu.crmType( save.getCrmType() );

        return contentRecommendMenu.build();
    }

    @Override
    public ContentRecommendEntity toContentRecommendEntity(ContentRecommend contentRecommend) {
        if ( contentRecommend == null ) {
            return null;
        }

        ContentRecommendEntity.ContentRecommendEntityBuilder contentRecommendEntity = ContentRecommendEntity.builder();

        contentRecommendEntity.crIdx( contentRecommend.getCrIdx() );
        contentRecommendEntity.contentRecommendMenu( toContentRecommendMenuEntity( contentRecommend.getContentRecommendMenu() ) );
        contentRecommendEntity.media( mediaToMediaEntity( contentRecommend.getMedia() ) );
        contentRecommendEntity.crSeq( contentRecommend.getCrSeq() );

        return contentRecommendEntity.build();
    }

    @Override
    public ContentRecommend toContentRecommend(ContentRecommendEntity save) {
        if ( save == null ) {
            return null;
        }

        ContentRecommend.ContentRecommendBuilder<?, ?> contentRecommend = ContentRecommend.builder();

        contentRecommend.createdAt( save.getCreatedAt() );
        contentRecommend.updatedAt( save.getUpdatedAt() );
        contentRecommend.crIdx( save.getCrIdx() );
        contentRecommend.contentRecommendMenu( toContentRecommendMenu( save.getContentRecommendMenu() ) );
        contentRecommend.media( mediaEntityToMedia( save.getMedia() ) );
        contentRecommend.crSeq( save.getCrSeq() );

        return contentRecommend.build();
    }

    @Override
    public List<ContentRecommend> toContentRecommendList(List<ContentRecommendEntity> allById) {
        if ( allById == null ) {
            return null;
        }

        List<ContentRecommend> list = new ArrayList<ContentRecommend>( allById.size() );
        for ( ContentRecommendEntity contentRecommendEntity : allById ) {
            list.add( toContentRecommend( contentRecommendEntity ) );
        }
    ...
    @Override
    public List<ContentRecommendMenu> toContentRecommendMenuList(List<ContentRecommendMenuEntity> list) {
        if ( list == null ) {
            return null;
        }

        List<ContentRecommendMenu> list1 = new ArrayList<ContentRecommendMenu>( list.size() );
        for ( ContentRecommendMenuEntity contentRecommendMenuEntity : list ) {
            list1.add( toContentRecommendMenu( contentRecommendMenuEntity ) );
        }
