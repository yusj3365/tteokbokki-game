...
@Component
public class MemberTokenInfraMapperImpl implements MemberTokenInfraMapper {

    @Override
    public MemberTokenEntity toEntity(MemberToken model) {
        if ( model == null ) {
            return null;
        }

        MemberTokenEntity.MemberTokenEntityBuilder memberTokenEntity = MemberTokenEntity.builder();

        memberTokenEntity.mtIdx( model.getMtIdx() );
        memberTokenEntity.member( memberToMemberEntity( model.getMember() ) );
        memberTokenEntity.mtDeviceToken( model.getMtDeviceToken() );

        return memberTokenEntity.build();
    }

    @Override
    public MemberToken toModel(MemberTokenEntity entity) {
        if ( entity == null ) {
            return null;
        }
        ...
        memberToken.createdAt( entity.getCreatedAt() );
        memberToken.updatedAt( entity.getUpdatedAt() );
        memberToken.mtIdx( entity.getMtIdx() );
        memberToken.member( memberEntityToMember( entity.getMember() ) );
        memberToken.mtDeviceToken( entity.getMtDeviceToken() );
