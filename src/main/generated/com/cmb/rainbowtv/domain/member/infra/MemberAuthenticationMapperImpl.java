...
import com.cmb.rainbowtv.domain.auth.infra.data.entity.MemberAuthenticationEntity;
...
import com.cmb.rainbowtv.domain.member.model.MemberAuthentication;
...
@Component
public class MemberAuthenticationMapperImpl implements MemberAuthenticationMapper {

    @Override
    public MemberAuthenticationEntity toEntity(MemberAuthentication memberAuthentication) {
        if ( memberAuthentication == null ) {
            return null;
        }

        MemberAuthenticationEntity.MemberAuthenticationEntityBuilder memberAuthenticationEntity = MemberAuthenticationEntity.builder();

        memberAuthenticationEntity.mauIdx( memberAuthentication.getMauIdx() );
        memberAuthenticationEntity.mauReqno( memberAuthentication.getMauReqno() );
        memberAuthenticationEntity.member( memberToMemberEntity( memberAuthentication.getMember() ) );
        memberAuthenticationEntity.mauName( memberAuthentication.getMauName() );
        memberAuthenticationEntity.mauBirth( memberAuthentication.getMauBirth() );
        memberAuthenticationEntity.mauCode( memberAuthentication.getMauCode() );
        memberAuthenticationEntity.mauTime( memberAuthentication.getMauTime() );

        return memberAuthenticationEntity.build();
    }

    @Override
    public MemberAuthentication toModel(MemberAuthenticationEntity entity) {
        if ( entity == null ) {
            return null;
        }

        MemberAuthentication.MemberAuthenticationBuilder<?, ?> memberAuthentication = MemberAuthentication.builder();

        memberAuthentication.createdAt( entity.getCreatedAt() );
        memberAuthentication.updatedAt( entity.getUpdatedAt() );
        memberAuthentication.mauIdx( entity.getMauIdx() );
        memberAuthentication.mauReqno( entity.getMauReqno() );
        memberAuthentication.mauName( entity.getMauName() );
        memberAuthentication.mauBirth( entity.getMauBirth() );
        memberAuthentication.mauCode( entity.getMauCode() );
        memberAuthentication.mauTime( entity.getMauTime() );
        memberAuthentication.member( memberEntityToMember( entity.getMember() ) );

        return memberAuthentication.build();
    }
